...
const userRegister = async (req, res) => {
    try {
        const { name, mobile, address, about, email, password, image, accountType } = req.body;
        ...
        const newUser = new UserModel({ name, mobile, address, about, email, password: hashedPassword, image, accountType });
...
const userLogin = async (req, res) => {
    try {
        ...
        const { name, mobile, address, about, image, accountType } = findData;

        res.status(200).json({ msg: "User login success", token, name, mobile, address, about, email, image, accountType });
    } catch (error) {
        console.error(error.message);
        res.status(500).json({ msg: "Server error" });
    }
