...
//         const suggestions = users.map(user => user.name, user.address, user.pincode);
...
const userSuggestion = async (req, res) => {
    try {
        const { name, address, pincode } = req.query;
        ...
        // Find users with the given criteria
        const users = await UserModel.find(query)
            .limit(10)
            .select('name address pincode -_id');

        // Map users to extract the desired fields
